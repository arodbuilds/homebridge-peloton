...
- The Devices line depended on every attached_devices entry carrying a string id and a name: a numeric or missing id folded devices into one, and a nameless device was listed as nothing. Devices are now keyed by id, or by name when there is none, stored on the owner's record at connect and at the daily check-in, and /status lists them all; a device without a name is listed by its hardware family.
- Household profiles showed the username as the display name and were never updated after they were written. The profile is "first_name last_name" from shared_user_set when present, username otherwise, and every subscriptions read brings a stored profile up to date; a member's own connected record replaces a display name that was only the username with the profile's name.
