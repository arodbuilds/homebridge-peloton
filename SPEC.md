...
10. Beta first: 0.1.0-beta.1 to the npm beta tag (the version homebridge-notify-switch started at), tested on Alex's Pi, then a tester call, then 1.0.0. Done: 1.0.0 is the first general release, to the npm latest tag, on September 17, 2026.
11. Heart-rate zone triggers are not offered on the settings page from 0.1.0-beta.2: the offering needs more refinement before it is offered (open item 3 in section 15). The suppression is in the page only: with one type left, Add trigger creates a Workout card directly and there is no chooser; a Heart-rate zone trigger already in config still renders, with the badge "Not offered in this release" after its type badge, and can be opened, edited, duplicated and removed. The runtime, config.schema.json, the rules (8.3), the poller (8.1), the accessories (9) and their tests keep the hrZone type exactly as they are, so a stored trigger keeps working.
12. Device codes only. What the plugin keeps about the hardware and apps it has seen is the device_type and platform pair and the fitness discipline of the workout it was first seen on (7, 8.2), never a workout id, title, date, or account detail, and a device report carries those codes and the plugin version alone (10). Unknown codes are named in later releases from those reports; the plugin never guesses a device from anything else.
...
{ "userId": "...", "username": "...", "displayName": "...", "imageUrl": "...", "isProfileImageDefault": false,
  ...
  "devicesSeen": [{ "deviceType": "home_bike_plus", "platform": "home_bike", "discipline": "cycling" }],
...
- devicesSeen is every device_type and platform pair the poller has observed on the membership, each with the fitness discipline of the workout it was first seen on, in order of first sighting (8.2). The poller writes it on the owner's record (isOwner true) when the store knows one, else on the polled account's record; /devices-seen reads the union across records (10). Nothing else about a workout is kept for it (2, item 12).
...
- Devices seen: every workout a poll returns, list or locked and imports included, has its device_type and platform pair recorded the first time it is seen (7): the pairs already stored are read once from the store and kept in memory, a new pair is appended to the owner's record with the workout's fitness_discipline, and a failure to write it is logged at debug and is not a poll failure. A pair whose device_type has no display name (src/devices.ts) is unknown, and its first sighting logs one info line, "New Peloton device seen: device_type={x}, platform={y}. Open the settings page, Advanced, Devices seen, to report it."; a known code is recorded silently. A pair is never reported twice, across restarts included.
...
| /devices-seen | none | devices[] (deviceType, platform, discipline, name, known): every pair in devicesSeen across the stored records (7), in order of first sighting and without duplicates; name is the display name from src/devices.ts or the raw code, known is true when the code has one; an entry without string codes is left out |
...
Page-side host contract: the Homebridge UI wraps the page in its own document inside a modal iframe and sizes the iframe to document.body.scrollHeight (plus 10 px) whenever the plugin-ui-utils client posts it, and links its own stylesheet (which sets `html, body { height: 100% }`) into the iframe after the plugin's. The page runs on the Homebridge Plugin Shell (design/README.md, Shell): the shell stylesheet from homebridge-notify-switch v1.3.2 is copied into homebridge-ui/public/index.css without edits, and its rule `:root, :root > body { overflow: hidden; height: auto }` makes the iframe document a page that is never a scroll container: no fixed or percentage height anywhere, so the host's resize never posts a new, larger height, the banner's top margin stays inside the measured height, and a wheel gesture over the page goes to the host's modal rather than latching on a scroll range of the iframe's own. Nothing on the page calls scrollIntoView; moving focus is what brings a field into view. The page still calls the client's fixScrollHeight after every section render, every card toggle, and every summary box change so the host measures the new content at once; a host without it is left alone. Every colour is a host Bootstrap variable with the host's light value as the fallback, re-declared under the host's dark marker (`body.dark-mode`, `body[class*="config-ui-x-dark-mode"]`) by the shell stylesheet on the root element, where the page's own tokens read them; the host's dark themes grey every .btn-link with an important rule, which the shell's own rule of the same weight answers, reading each text button's intent (Remove red, the help toggle and Cancel secondary, the summary box entries the box's colour, everything else the link colour).

Page-side shell behaviour (design/README.md, Shell; the shell's rules cited by id): the summary box sits in the page flow after Settings and before the closing paragraph, in the warning tone with no shadow (F4); its entries read "{Card}: {message}" and, clicked, focus the named control itself, opening a collapsed trigger card or the Advanced disclosure first and rebuilding nothing (T5); an empty required field reads "{Label} is required." from the one label table the sections render from (W4); the unsaved draft in localStorage is written only once the user has changed something on the page, holds no passwords, is offered back only over a saved Peloton configuration it differs from, and is never offered on a page that opens with no saved Peloton configuration, where one left from before the first save is removed (M1). The Settings section is the intro and one collapsed Advanced disclosure holding, in this order, Name, Debug logging, Fast polling switch turns off after (minutes), Keep fast polling after a workout ends (minutes), Attention needed sensor, Daily check-in time, the read-only Devices seen block, and Restore from backup; it opens when anything inside differs from its default, when a field inside turns invalid, and when a summary entry targets one (C3). The Devices seen block lists every pair from /devices-seen (read at load and after every /status refresh) as "{display name} ({device_type}, {platform})" with an outlined Known tag or a warning Unknown tag, a Copy report button on each Unknown row, and the caption "The plugin keeps only the device codes it has seen (device type and platform), nothing about your workouts. Copying a report shares those two codes, the workout type such as cycling or strength, and the plugin version. No names, dates, class titles, or account details are included." followed by the link "Report it on GitHub" to the repository's new issue page with the device-report template preselected; with no pair it reads "None seen yet. The list fills in as the plugin polls." Copy report puts exactly "Unknown Peloton device", "device_type: {x}", "platform: {y}", "discipline: {fitness_discipline}", "plugin: homebridge-peloton {version}", one per line, on the clipboard (the async clipboard API, else a hidden textarea and the copy command, since the Homebridge UI is usually served over plain http) and toasts "Report copied. Paste it into the GitHub issue."; when neither works, the report text is shown under the row in the shell's status box with "Could not copy. Select the report below and copy it by hand." The block has no control and takes no part in validation. Trigger cards are collapsible: the whole header row toggles (click, Enter, Space), a chevron sits at the right edge, the link reads "Show settings" while collapsed and "Done" while open, the help toggle shows only while open, a muted summary of the key values follows the badges while collapsed, and the only trigger, a newly added one and a duplicate open expanded. Add trigger creates a Workout card directly and focuses its Name; there is no chooser while Heart-rate zone is not offered (2), and a stored Heart-rate zone card carries the badge "Not offered in this release" after its type badge. Three Peloton exceptions to the shell's contract stand and are recorded here: trigger cards have no footer primary action (C4 allows at most one; Peloton has none), Polling renders uncarded on the page grid although it is not the Settings section (R5), and the Checking pill spins (the shell allows no animation beyond button colour and the chevron).
...
- "{displayName}: workout started ({fitness_discipline}, {title})" where title is ride.title when present, else the workout's title, else its name; "{displayName}: workout started ({fitness_discipline})" when none of them is set, so an app class never logs an empty title
...
- "New Peloton device seen: device_type={x}, platform={y}. Open the settings page, Advanced, Devices seen, to report it.", once per unknown device_type and platform pair, ever (8.2)
...
- Poller tests use the injectable clock and scheduler and a route-based fake fetch, with the real account store in a temporary directory and the real API wrappers, and cover: standby with 0 and 120 s, scanning stagger, lock-on, locked polling through getWorkout rather than the list, a synced import mid-workout, stacked classes with hold, switch off during locked, auto-off timer anchoring, HR dwell on and off, stale samples, 401 then refresh, invalid_grant to reconnect_needed with the attention event, backoff on 5xx, the daily check-in, the devices seen (every pair once on the owner record, imports included, the info line once per unknown pair across a restart, no workout id or title in the record), the fast interval kept after a workout end (the window with the switch off, a second workout caught within it, 0 minutes, the switch on, a switch turned off inside the window, standby 0, stop), and the log title fallback.
- Rules tests cover every matching path in 8.3: the Bike+, Tread and Guide filters, Apple TV against platform apple_tv, the app filter against iOS_app (iPhone and iPad) and android_app and never against an Apple Health import or an unseen platform. A devices suite checks the display name table and the raw code for an unmapped one. Config tests cover every default, clamp, and generated id in 6. Accessory tests cover the service swap, the acknowledged write, and UUID stability with hap-nodejs objects; platform tests cover registration, orphan cleanup, skip lines, and event wiring with a fake Homebridge API.
- Store tests cover atomic write and rotation.
- Schema tests compile config.schema.json with ajv (a dev dependency) and check the section 6 sample, the defaults, and every constraint in 6. UI server tests run the handlers against an in-memory fake store and a routed fake fetch: every request in 10 (/devices-seen with the union across records, the display names and the known flag, duplicates and malformed entries dropped), every stage mapping, the household profile claim, browser start and finish through the real PKCE exchange including expiry, the avatar 204 path and cache, the last workout time, and the one-hour household refresh on /status (a stale or missing householdFetchedAt reads once and then serves the store for an hour, a failed read keeps the stored household, a member is never asked). The API tests parse the live-shaped subscriptions fixture, a null device_name, a missing device_group, and a member without a name. The page model (homebridge-ui/public/model.js) is imported into node:test for the account write-back, trigger defaults, validation messages (an empty required field reads "{Label} is required." from the label table), the copy rules, the Devices line labels, the device report text, and the keep-fast validation; the draft suite (test/ui-draft.test.mjs) covers the draft written without passwords, read back only while fresh and well formed, and removed otherwise. The page suite renders the Devices line with a device that has no name, checks that the page asks the host to size the iframe after every render and card toggle, covers the prefilled trigger names (a new Workout card named "Workout"; the zone-following name of a new Heart-rate zone card stays in the page model for when the offering returns and is not reachable from the page), Add trigger creating a Workout card directly with no chooser, a stored Heart-rate zone card with its "Not offered in this release" badge that can be opened, edited and duplicated, the Settings fields under Advanced in order (the keep-fast field with its default, floor and caption; the Devices seen block between Daily check-in time and Restore from backup with its rows, tags, Copy report on Unknown rows only, the caption and the GitHub link, the exact copied text through the clipboard, the copy-by-hand fallback, and the empty line) and the disclosure opening for a changed, invalid or targeted field, the trigger card header (collapsed on load with the summary and "Show settings", the whole row and the keyboard toggling in place, the help toggle only while open, the only, new and duplicated card open, the footer without a primary action, every field in a grid cell), the summary box in flow whose entries focus the control itself without rebuilding the section, the draft written only after a change and never offered on a page with no saved configuration, the switch Name always shown, the estimate counting Connected and Checking accounts, and the callout's Dismiss link. Layout, scrolling, and theme colours are checked in headless Chromium with the host's stylesheet and body classes, the page inside an iframe in a scrolling modal body sized to the posted height plus 10 px: in both themes at 900 and 400 px, documentElement.scrollHeight equals clientHeight, the posted height settles without a resize loop, and the first wheel gesture at the top and at the bottom of the page scrolls the modal while the iframe document stays put (the harness is not in the repository); the page itself is also checked by hand in the Homebridge UI.
...
- 1.0.0 to the npm latest tag through the GitHub release workflow: a GitHub release marked Latest publishes with `npm publish --tag latest --provenance`, a release marked pre-release with `--tag beta` (confirmed for 1.0.0; the betas went there). The workflow checks that the release tag matches package.json. Keywords: homebridge-plugin, homebridge, supports-hap, peloton, homekit, workout, heart-rate, fitness. The plugin icon is not a package.json field: the Homebridge UI takes icons from its central plugin list, which is settled at verification.
- README (1.0.0): the banner with the tagline "HomeKit sensors driven by Peloton workouts: workout in progress and heart-rate zones.", the badges row (npm version on the latest tag, npm downloads, license, the build badge; the verified badge commented out until verification), a one-paragraph purpose, the not-affiliated line, the status line "stable. 1.0.0 is the first general release.", requirements (Homebridge 1.8 or 2.x, Node 20 or later, a Peloton membership), install, the setup walkthrough with four of the settings page screenshots under assets/screenshots/ (accounts, trigger-workout, polling, settings; trigger-zone.png stays in the folder for when the offering returns), the sentence "Heart-rate zone triggers are being refined and are not offered in this release." under the Workout trigger step, the new Advanced fields, the How I use it section after Setup, browser sign-in with the Back-button and history alternatives, how it works, the Devices section with the display names and codes from 4.2 and the line "Row and Android: not yet observed; the Devices seen block under Advanced makes a report a copy and paste.", privacy and what is stored where (the devices seen among it), troubleshooting, development, and credits naming each project's license and that no code is shared. The hand-edited config.json appendix is gone; the README points at section 6 and config.schema.json instead.
- CHANGELOG from beta.1, grouped Added, Changed, Fixed. The release notes of a version are its CHANGELOG section alone.
- CodeQL default setup reads .github/codeql/codeql-config.yml, which ignores design/**, assets/**, and test/**; the build workflow runs with contents: read. The npm package carries only the files list in package.json (dist, homebridge-ui/public, homebridge-ui/server.js, config.schema.json, LICENSE, README.md, CHANGELOG.md), so design/, test/, and the screenshots are not published.
- Verification submission only after 1.0.0 and a soak.

## 15. Open items to settle during beta

Only what is still open after 1.0.0. Everything settled before is in the body, with the notes in section 16.

1. Row and Android codes. The Row's device_type and platform and the Android app's device_type have not been observed (no Row and no Android device on the test membership). A Row workout matches only "any" until its codes are known and a Row option can be added; the app filter already accepts platform android_app on the expectation that the Android app reports it. The Devices seen block and the device report form exist to settle both from a user's report.
...
3. The Heart-rate zone offering. The trigger type works in the runtime and a stored trigger still renders and can be edited, but the offering on the settings page needs more refinement before it is offered (2, item 11); until then Add trigger creates Workout cards only.
...
### 0.1.0-beta.2, PR 1 clarifications

The settings page aligned with the Homebridge Plugin Shell as homebridge-notify-switch v1.3.2 ships it (design/README.md, Shell, and design/HANDOFF.md, BUILD-CONTRACT.md and BUILD-CONTRACT-DEFINITIONS.md copied from that tag). Each is a clarification, not a change to section 2.

- 9: nothing in this section changes; the accessories, their names and their reconciliation are as before. The section is named in the scope of this PR and was read for that, and the settings page moves no field that the accessories read.
- 10: the page-side host contract is restated on the shell's scroll rule (`:root, :root > body { overflow: hidden; height: auto }` replaces the build 4 `html, body` rule, and no scrollIntoView remains), the dark-mode token fallbacks, and the shell behaviour the page adopts: the in-flow summary box, entries that focus the control itself, "{Label} is required." from the label table, the draft written only after a change and never offered on a fresh page, Settings entirely under Advanced in the fixed order, and the collapsible trigger card header. The three Peloton exceptions to the shell contract (no card primary action, Polling uncarded, the Checking spinner) are recorded there.
- 13: the page, model and draft suites named above, and what the Chromium harness checks.
- design/README.md: which files under homebridge-ui/public/ are the shell's and which are Peloton's, the shell source tag, how each item of the alignment inventory was resolved, and the states the build settled (listed there under Clarifications from beta.2, PR 1). The Guide device option, the Heart-rate zone suppression and the version bump are PR 2 and stay out of this one.

### 0.1.0-beta.2, PR 2 clarifications

The second PR of beta.2: the Guide device option, the Heart-rate zone offering withdrawn from the settings page, the provenance note, the README badges and credits, CodeQL, and the version. Each is a clarification, not a change to the decisions in section 2; item 11 there is new and records a release-scoped decision, not a reversal of any earlier one.

- 2: item 11 records that Heart-rate zone triggers are not offered on the settings page from 0.1.0-beta.2, with the reason (the offering needs more refinement) and the boundary of the suppression (the page only; the runtime, schema, rules, poller, accessories and tests keep the type).
- 4.2: the device table gains the Guide (device_type t21n8m2, platform tiger, observed 11 September 2026), so the table's heading no longer says ride tests.
- 6 and 8.3: device is any, bike, tread, or guide, and "guide" matches platform tiger; config.schema.json offers Guide with the caption "Bike is a ride on the Bike or Bike+, Tread is a workout on the Tread or Tread+, Guide is a class taken on the Peloton Guide."
- 10: Add trigger creates a Workout card directly and focuses its Name, with no chooser; a stored Heart-rate zone card carries the badge "Not offered in this release".
- 13: the rules tests cover the Guide filter; the page suite covers Add trigger without a chooser and the stored zone card with its badge; the zone-following name stays in the page model, unreachable from the page.
- 14: the README badges row, the four screenshots the walkthrough uses, the credits with licenses, the release notes as the CHANGELOG section alone, the CodeQL configuration and the build workflow's permissions, and the npm package contents.
- 15: the Guide item is closed; Row hardware, verification-required accounts and the Heart-rate zone offering remain.
- src/auth/peloton-auth.ts: a provenance note in the header comment (the flow written from observation on September 10 to 11, 2026, cross-checked against peloton-to-garmin, the Home Assistant Peloton integration and @dofek/peloton, no code taken).
- design/README.md: Add trigger without the chooser, the badge on a stored Heart-rate zone card, the Guide option and caption, the version, and the screenshot the README no longer uses (listed there under Clarifications from beta.2, PR 2).

### 1.0.0 clarifications

The first general release (September 17, 2026): the Apple TV and Phone or tablet app device options, the device name table, the devices seen and the device report, the fast interval kept after a workout ends, the log title fallback, the README and the version. Each is a clarification, not a change to the decisions in section 2; item 10 there records that the beta plan was carried out, and item 12 is new and pins what the plugin keeps about devices.

- 2: item 10 notes the release; item 12 records that only device codes are kept and reported.
- 4.2: the device table carries every code observed between September 11 and 16, 2026 (Bike+, Tread, Guide, the Apple TV, iPhone and iPad apps, an Apple Health import) and the original Bike from public logs, with the filter each maps to; Row and Android stay open.
- 5: src/devices.ts, the one display name table, is in the module list.
- 6 and 8.3: device is any, bike, tread, guide, appletv, or app; appletv matches platform apple_tv and app matches iOS_app or android_app when the workout is Peloton originated, so an Apple Health import never matches it. advanced.keepFastAfterEndMinutes, default 5, floor 0.
- 7 and 8.2: devicesSeen on the owner's record, how the poller records a pair once and logs an unknown one once, and that a failure to write it is not a poll failure.
- 8.1 and 8.4: the fast interval kept after a workout end, its window counted from the end, and how it hands back to the switch state.
- 10: /devices-seen, the Devices seen block under Advanced with its copy, tags, the report text, the clipboard fallback and the GitHub link, and the two new entries in the Advanced order.
- 11: the workout started line's title fallback and the New Peloton device seen line.
- 13: the suites named above.
