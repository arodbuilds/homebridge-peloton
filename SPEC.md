...
Version: draft 6 for 0.1.0-beta.2, PR 2 of 2 (14 September 2026, the Guide device option, the Heart-rate zone offering withdrawn from the settings page, provenance, CodeQL and the version)
...
- README (build 4, beta.2 PR 2): the banner with the tagline "HomeKit sensors driven by Peloton workouts: workout in progress and heart-rate zones.", the badges row (npm version on the beta tag, npm downloads, license, the build badge; the verified badge commented out until verification), a one-paragraph purpose, the not-affiliated line, requirements (Homebridge 1.8 or 2.x, Node 20 or later, a Peloton membership), install, the setup walkthrough with four of the settings page screenshots under assets/screenshots/ (accounts, trigger-workout, polling, settings; trigger-zone.png stays in the folder for when the offering returns), the sentence "Heart-rate zone triggers are being refined and are not offered in this release." under the Workout trigger step, browser sign-in with the Back-button and history alternatives, how it works, privacy and what is stored where, troubleshooting, development, credits naming each project's license and that no code is shared, and the status line "Beta. Please report what works and what does not." The hand-edited config.json appendix is gone; the README points at section 6 and config.schema.json instead.
...
Only what is still open after 0.1.0-beta.2. Everything settled before is in the body, with the notes in section 16.

1. Row hardware. The Row's device_type and platform codes have not been observed (no Row on the test membership), so the Device field stays at any, bike, tread, and guide, and a Row workout matches only "any" until the codes are known.
