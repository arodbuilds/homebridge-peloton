...
5. The final redirect to members.onepeloton.com/callback carries code and state. Auth0 restores the state from the authorize URL here, so the module compares against that state, not the transaction state. The module does not follow the redirect; it extracts the code and exchanges it.
